...
# Submit button
Button(text="Submit", command=lambda: print(f"The name of the person is {name_value.get()},\nthe phone number is {phone_value.get()},\nthe email address is {email_value.get()},\nthe gender is {gender_value.get()},\nthe payment mode is {payment_mode_value.get()},\nand the food services are {food_services_value.get()}")).grid(row=7, column=3)     # Lambda function is used to pass arguments to function without calling it.
